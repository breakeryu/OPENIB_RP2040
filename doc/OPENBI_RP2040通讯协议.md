...
|   E004H    |        加速度Ax（单位：m/s^2）         |  ——  |
|   E005H    |        加速度Ay（单位：m/s^2）         |  ——  |
|   E006H    |        加速度Az（单位：m/s^2）         |  ——  |
|   E007H    |         角速度Wx（单位：°/s）          |  ——  |
|   E008H    |         角速度Wy（单位：°/s）          |  ——  |
|   E009H    |         角速度Wz（单位：°/s）          |  ——  |
|   E00AH    |           角度x轴（单位：°）           |  ——  |
...
| 寄存器地址 |                 数据类型                 | 系数 |
| :--------: | :--------------------------------------: | :--: |
|   E050H    |   通讯系统配置时间超时阈值（单位：ms）   |  ——  |
|   E051H    | 逻辑控制系统配置时间超时阈值（单位：ms） |  ——  |
|   E052H    |       蓝牙串口波特率（单位：baud）       |  ——  |
|   E053H    |      RS485串口波特率（单位：baud）       |  ——  |
|   E054H    |              设备硬件版本_H              |  ——  |
|   E055H    |              设备硬件版本_L              |  ——  |
|   E056H    |              设备软件版本_H              |  ——  |
|   E057H    |              设备软件版本_L              |  ——  |
...
|   E109H    | **Channel_3输入模式配置**，写入值对应功能如下：<br />⏹模拟电压输入 0x00        ⏹模拟电流输入 0x01   ⏹数字输入  0x02        <br />❗注意：当输入Channel_3输入使能为0时，该通道关闭，这种情况下，即使配置通道功能，通道配置也会失效！要想配置通道，要首先使本通道的输入使能配置为1。 |  ——  |
|   E10AH    | **Channel_4输入模式配置**，写入值对应功能如下：<br />⏹模拟电压输入 0x00        ⏹模拟电流输入 0x01   ⏹数字输入  0x02            <br />❗注意：当输入Channel_4输入使能为0时，该通道关闭，这种情况下，即使配置通道功能，通道配置也会失效！要想配置通道，要首先使本通道的输入使能配置为1。 |  ——  |
|   E10BH    | **继电器通道数配置**，写入值对应功能如下：<br />⏹全部开启 0x00        ⏹使能1路继电器 0x01   ⏹使能两路继电器  0x02  ⏹使能三路继电器  0x03 |  ——  |

**注：读出的数据应该乘以系数：以模拟输出电压为例：模拟输出电压=模拟输出电压实际值*1**
